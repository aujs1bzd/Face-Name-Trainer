...
export default function App() {
  ...
  return <div className="app-shell"><header><h1>Face Name Trainer</h1><nav aria-label="メインナビゲーション">{['home','form','list','quiz','settings'].map((p) => <button className={page === p ? 'active' : ''} aria-current={page === p ? 'page' : undefined} key={p} onClick={() => p === 'quiz' ? startQuiz(mode) : (setPage(p as Page), setEditing(null))}>{({home:'ホーム',form:'人物登録',list:'人物一覧',quiz:'クイズ',settings:'設定'} as Record<string,string>)[p]}</button>)}</nav></header><main>{message && <p className="notice" role="status">{message}</p>}
    {page === 'home' && <section><h2>ダッシュボード</h2><div className="cards"><b>登録人数<br /><span>{people.length}</span></b><b>今日復習<br /><span>{dashboard.due}</span></b><b>総回答数<br /><span>{dashboard.total}</span></b><b>正答率<br /><span>{dashboard.accuracy}%</span></b></div><h3>苦手な人物</h3>{dashboard.weak.map((p) => <article className="row" key={p.id}><img src={p.imageDataUrl} alt="" /><span>{p.displayName}（誤答 {p.stats.wrongAttempts} / 正答率 {rate(p)}%）</span></article>)}</section>}
    ...
    {page === 'list' && <section><h2>人物一覧</h2><div className="filters"><input aria-label="名前検索" placeholder="名前検索" value={query} onChange={(e) => setQuery(e.target.value)} /><input aria-label="所属検索" placeholder="所属検索" value={teamQuery} onChange={(e) => setTeamQuery(e.target.value)} /></div>{people.filter((p) => `${p.displayName}${p.firstName}${p.lastName}${p.kana}`.includes(query) && p.team.includes(teamQuery)).map((p) => <article className="person" key={p.id}><img src={p.imageDataUrl} alt="" /><div className="person-info"><h3>{p.displayName}</h3><p>{p.kana} / {p.team}</p><p className="person-stats">正答率 {rate(p)}%・最終 {fmt(p.stats.lastReviewedAt)}・次回 {fmt(p.stats.nextReviewAt)}</p><div className="person-actions"><button onClick={() => { setEditing(p); setPage('form'); }}>編集</button><button className="danger" onClick={async () => confirm('削除しますか？') && (await deletePerson(p.id), await refresh())}>削除</button></div></div></article>)}</section>}
    {page === 'quiz' && <section className="quiz"><h2>この人の名前は？</h2><div className="mode"><button className={mode === 'input' ? 'active' : ''} onClick={() => startQuiz('input')}>自由入力</button><button className={mode === 'multiple' ? 'active' : ''} onClick={() => startQuiz('multiple')}>4択</button></div>{quizPerson && <><img className="face" src={quizPerson.imageDataUrl} alt="名前を答える人物" />{result === null ? mode === 'input' ? <form onSubmit={(e) => { e.preventDefault(); const ok = isCorrectAnswer(answer, quizPerson); submitAnswer(ok, ok ? null : findPersonByAnswer(answer, people)); }}><input autoFocus enterKeyHint="done" value={answer} onChange={(e) => setAnswer(e.target.value)} placeholder="名前を入力" aria-label="回答する名前" /><button>回答</button></form> : <div className="choices">{choices.map((c) => <button key={c.id} onClick={() => submitAnswer(c.id === quizPerson.id, c.id === quizPerson.id ? null : c)}><ruby>{c.displayName}{c.kana && <><rp>（</rp><rt>{c.kana}</rt><rp>）</rp></>}</ruby></button>)}</div> : <div className={result ? 'correct' : 'wrong'}><h3>{result ? '正解！' : '不正解'}</h3><p className="answer">正しい名前: {quizPerson.displayName}</p><p>{quizPerson.kana}</p><p>{quizPerson.team}</p><p>{quizPerson.memo}</p>{!result && mistakenPerson && <div className="mistaken-person"><p>それはこの人です</p><img src={mistakenPerson.imageDataUrl} alt={mistakenPerson.displayName} /><strong>{mistakenPerson.displayName}</strong></div>}<button className="next-button" onClick={nextQuestion}>次へ</button></div>}</>}</section>}
...
function PersonForm({ person, onSave, onCancel }: { person: Person; onSave: (p: Person) => void; onCancel: () => void }) {
  ...
  return <section><h2>人物登録・編集</h2><form className="form" onSubmit={submit}><label>顔写真<input type="file" accept="image/*" onChange={async (e) => e.target.files?.[0] && update('imageDataUrl', await readFile(e.target.files[0]))} /></label>{draft.imageDataUrl && <img className="preview" src={draft.imageDataUrl} alt="顔写真のプレビュー" />} {(['lastName','firstName','displayName','kana','team'] as const).map((k) => <label key={k}>{({lastName:'姓',firstName:'名',displayName:'表示名',kana:'ふりがな',team:'所属・チーム'} as Record<string,string>)[k]}<input value={draft[k]} onChange={(e) => update(k, e.target.value)} /></label>)}<label>メモ<textarea value={draft.memo} onChange={(e) => update('memo', e.target.value)} /></label><label>別名・許容回答（1行1件）<textarea value={draft.aliases.join('\n')} onChange={(e) => update('aliases', e.target.value.split('\n').map((x) => x.trim()).filter(Boolean))} /></label><div className="form-actions"><button disabled={!draft.imageDataUrl}>保存</button><button type="button" className="secondary" onClick={onCancel}>キャンセル</button></div></form></section>;
}
...
function Settings({ people, refresh }: { people: Person[]; refresh: () => Promise<void> }) {
  ...
  return <section><h2>設定 / データ管理</h2><p>データはIndexedDBに保存され、外部APIやサーバーへ送信しません。</p><div className="settings-actions"><button onClick={exportJson}>JSONエクスポート</button><label className="import">JSONインポート<input type="file" accept="application/json" onChange={(e) => e.target.files?.[0] && importJson(e.target.files[0])} /></label><button className="danger" onClick={async () => confirm('全データを削除しますか？') && (await clearPeople(), await refresh())}>全削除</button></div></section>;
}
